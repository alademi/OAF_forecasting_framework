...
def evaluate_model(base_model, model_name, test, dataset_name):
...
def detect_concept_drift(validation_set, mean_ref, hoeffding_bound, models, kmeans, model_name, file_name):
    ...
    if delta > hoeffding_bound:
        ...
        #new_kmeans = fuse_clusters(kmeans, new_kmeans)
        models = models + new_models

    return mean_ref, models, new_kmeans, new_validation_windows


def evaluate_online(finetuned_models, model_name, file_name, val, test, kmeans):
    test_windows, test_labels = util.make_windows(test, WINDOW_SIZE, HORIZON)
    ...
    for t in range(len(test_windows)):
        ...
        drift_points += 1
        mean_ref, finetuned_models, new_kmeans, new_validation_windows = detect_concept_drift(val, mean_ref,
                                                                                    hoeffding_bound, finetuned_models, kmeans, model_name, file_name,
                                                                                    )
        if new_kmeans is not None:
            ...
            # plot_cluster_comparison(current_kmeans, new_kmeans, current_validation_window, new_validation_windows, drift_points)
            drift_points = 0
            #current_validation_window = new_validation_windows
            #current_kmeans = new_kmeans
            print(f"detected at time {t}")
...
def evaluate_offline(finetuned_models, model_name, file_name, test, cluster_centers):
    predictions = []
    test_windows, test_labels = util.make_windows(test, WINDOW_SIZE, HORIZON)
    for t in range(len(test_windows)):
        recent_window = test_windows[t]
...
def save_results_to_csv(dataset_name, results):
    """
    Save evaluation results for all models in the dataset to a single CSV file.
    ...
    fieldnames = ["Model", "Base", "Offline", "Online", "Online Runtime"]
    file_exists = os.path.isfile(results_file)

    with open(results_file, mode='w', newline='') as csv_file:  # Overwrite after each dataset
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()  # Always write header (overwrite each dataset run)
    ...
    for name in file_names:
        if name.lower().endswith('.csv'):
            ...
            for model_name in MODELS:
                ...
                result_online = evaluate_online(finetuned_models, model_name, dataset_name, val, test, clustering_results)
                end_time = time.time()
